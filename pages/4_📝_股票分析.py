...
<div style="font-weight:700;font-size:16px;color:var(--text-color);margin-bottom:8px;">📋 初始分析</div>
<div style="font-size:14px;color:var(--text-color);line-height:1.6;">{html.escape(ps_dto.initial_analysis)}</div>
</div></div>""",
...
with col_back:
    if st.button("← 返回股票池", use_container_width=True):
...
# ============================================================
# 历史记录（左 2/3）+ 添加新分析（右 1/3）
# ============================================================

hist_col, add_col = st.columns([2, 1])
...
with hist_col:
    ...
    if not notes:
        render_empty_state("暂无分析记录，使用右侧表单添加第一条记录", icon="📝")
    else:
        for n in notes:
            # 卡片本体（含头部操作按钮 + 内容 + 配图）
            with st.container(border=True):
                ...
                with h[1]:
                    if n.user_id == user["id"]:
                        if st.button("✏️", key=f"edit_btn_{n.id}", use_container_width=True, help="编辑"):
                            st.session_state[f"edit_{n.id}"] = True
                            st.rerun()
                with h[2]:
                    if n.user_id == user["id"]:
                        if st.button("🗑️", key=f"del_btn_{n.id}", use_container_width=True, help="删除"):
